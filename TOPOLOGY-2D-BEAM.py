########################################################################################################
### MICHEL BEAM 2D: BENCHMARK FOR TOPOLOGY OPTIMIZATION                                              ###
...
# Mesh szize
meshSize=10 ## Mesh size in mm

# Geometry
x=1000 ## 1/2 Width in mm
y=200 ## 1/2 Height in mm
LS=20 ## Length of support in mm 

# Geometry restrictions
minSize=0 ## Minimum thickness of members in design area in mm
minSizeTop=0 ## Minimum thickness of the top floor in mm
planarSym=False ## Planar symmetry (for axis 1)
demold=False ## Demold control

# Material properties
E=210000 ## E-modulus in N/mm2
p=0.3 ## Poison ratio 

# Optimization parameters
VF=0.5 ## Volume fraction (for design area)
...
maxDeltaDens=0.25 ## Maximum change per design cycle (default=0.25)
initialDens=0.5 ## Initial density (default=DEFAULT)
## Convergence
conObjective=0.001 ## Objective function delta criterion (default=0.001)
conDensity=0.005 ## Element density delta criterion (default=0.005)
## Cycles
mDC=1000 ## Maximum number of cycles

# Computer settings
numCores=2 # Number of cores that will be uese of the processor
perMem=90 # Percentage of the memory that will be uses of the computer in %
...
nTiles=100 # Number of edges to apply a load (top will be divided in n)

# Settings for all load cases
tilePressure=50 ## Pressure
...
## Load case 1
LC1_T25=tilePressure
LC1_T26=tilePressure
## Load case 2
LC2_T75=tilePressure
LC2_T76=tilePressure
...
for a in range(0, nTiles):
    ...
    modra.Surface(name='LOAD-TILE-%s' % (a+1), side1Edges=modrain.edges.findAt((findat, ))) 
...
# Create constraints (rigid body)
for i in [1,2]:
    mod.RigidBody(name='CONSTRAINT-BC-%s' % i, 
        pinRegion=modra.sets['EDGE-BC-%s' % i], 
...
for lc in range(1, nLoadCases+1):
    ...
    for tile in range(1, (nTiles+1)):
        if globals()["LC%s_T%s" % (lc, tile)]!=0:
            mod.Pressure(createStepName='STEP-1', region=modra.surfaces['LOAD-TILE-%s' % tile], 
                name='LOAD-TILE-%s' % tile, magnitude=globals()["LC%s_T%s" % (lc, tile)]) 
