...
optJobName='%s-JOB' % processName ## Job name for topology optimization
...
# Elements
nelx=int(150)
nely=int(1) # nely=1 for a 2D beam! 
nelz=int(25)

# Geometry
sizeBC=float(meshSize*0) ## Size of the surface of the BCs in mm
fixy=True ## Fix all nodes in y-direction (True or False)

# Geometry restrictions
minSize=1.5*meshSize ## Minimum thickness of members in mm
minSizeTop=0 ## Minimum number of elements of the top layer
freezeCellsBC=False ## Freeze cells at boundary condition
planarSym1=True ## Planar symmetry for axis 1
planarSym2=True ## Planar symmetry for axis 2
demold=True ## Demold control
demoldAngle=0 ## Draft angle for demolding in degrees (0-20)
...
volumeFraction=0.5 ## Volume fraction range 0..1 (for design space)
operation='SUM' ## MAX, MIN or SUM: Operation between steps and load cases

# Computer settings
numCores=1 # Number of cores that will be uese of the processor
perMem=90 # Percent of the memory that will be uses of the computer
...
maxDeltaDens=0.2 ## Maximum change per design cycle (default=0.25)
initialDens=DEFAULT ## Initial density (default=DEFAULT)
## Convergence
### Stops when both criteria are reached. 
conObjective=0.99 ## Objective function delta criterion (default=0.001)
conDensity=0.001 ## Element density delta criterion (default=0.005)
## Cycles
mDC=999 ## Maximum number of cycles

# Submit
submitJob=False ## Normal job to check the model (only for LC-1)
submitOpt=True ## Submitting the optimization task
...
nTilesX=int(nelx) ## Number of tiles in the top layer in x-direction
nTilesY=int(1) ## Number of tiles in the top layer in y-direction

# Set load for all tiles in top layer
for lc in range(1, nLoadCases+1):
    for tile in range(1, (nTilesX*nTilesY)+1):
        globals()["LC%s_T%s" % (lc, tile)]=0.0

# Set load for specific tile and load case
## Load case 1
globals()["LC1_T{}".format(int(nelx/2))]=0.5
globals()["LC1_T{}".format(int(nelx/2)+1)]=0.5
...
for y in range(0, nTilesY):
    for x in range(0, nTilesX):
        ...
        n=(x+1)+(nTilesX*y)
        ## Create surface
        modpa.Surface(name='LOAD-TILE-%s' % n, side1Faces=modpa.faces.findAt((findat, )))
        del findat, n
...
if sizeBC!=0:
    for i in range(1, len(listRP)+1):
        mod.RigidBody(name='CONSTRAINT-BC-%s' % i, pinRegion=modrain.sets['FACE-BC-%s' % i], 
            refPointRegion=modra.sets['RP-%s' % i])
...
if nLoadCases>1:
    for num in range(2, nLoadCases+1):
        mdb.Model(name='LC-%s' % num, objectToCopy=mdb.models[modelName])
...
for lc in range(1, nLoadCases+1):
    mod=mdb.models['LC-%s' % lc]
    modra=mod.rootAssembly
    for tile in range(1, (nTilesX*nTilesY)+1):
        if globals()["LC%s_T%s" % (lc, tile)]!=0:
            mod.Pressure(createStepName='STEP-1', name='LOAD-TILE-%s' % tile, 
                magnitude=globals()["LC%s_T%s" % (lc, tile)], distributionType=TOTAL_FORCE, 
                region=modrain.surfaces['LOAD-TILE-%s' % tile])
...
# Topology process
## Save all cycles
mdb.OptimizationProcess(dataSaveFrequency=OPT_DATASAVE_EVERY_CYCLE, maxDesignCycle=mDC, 
    model=modelName, name=processName, odbMergeFrequency=2, prototypeJob=optJobName, task=taskName) 
'''
## Save initial, first and last cycle 
mdb.OptimizationProcess(dataSaveFrequency=OPT_DATASAVE_SPECIFY_CYCLE, saveEvery=None, saveFirst=True, 
